...
class Processor(QObject):  
    ...
    def __init__(self, files, value, timeout, delay):
        super().__init__()
    ...
    def scanner_01(self, execaddress):  

        if not self.Flags:
            try:
                ipaddress = self.separate(execaddress)[0]  
                enaddress = self.separate(execaddress)[1] 
                ...
                response = c.recv(1024).decode("iso-8859-1")  
    ...
    def scanner_02(self, execaddress):  
        if not self.Flags:
            try:
                ipaddress = self.separate(execaddress)[0]  
                enaddress = self.separate(execaddress)[1]  
    ...
    def check(self, i, file):  
        for ip in file:
            self.scanner_01(ip.strip()) 
            self.scanner_02(ip.strip())  
        else:
            self.Flags = True
